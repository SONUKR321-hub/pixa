...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, removeBackground, generateNewBackground } from './services/geminiService';
...
type Tab = 'retouch' | 'background' | 'adjust' | 'filters' | 'crop';
...
const App: React.FC = () => {
  ...
  const handleRemoveBackground = useCallback(async () => {
    ...
    try {
        const transparentImageUrl = await removeBackground(currentImage);
        const newImageFile = dataURLtoFile(transparentImageUrl, `bg-removed-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
  ...
  const handleGenerateBackground = useCallback(async (backgroundPrompt: string) => {
    if (!currentImage) {
      setError('No image loaded to add a background to.');
      return;
    }
    ...
    try {
        const newBgImageUrl = await generateNewBackground(currentImage, backgroundPrompt);
        const newImageFile = dataURLtoFile(newBgImageUrl, `new-bg-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to generate new background. ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'background', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'background' && <BackgroundPanel onRemoveBackground={handleRemoveBackground} onGenerateBackground={handleGenerateBackground} isLoading={isLoading} />}
